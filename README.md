...
[`submit_to_condor.py`](submit_to_condor.py) is your primary interaction point
with this suite. It takes a name for your job, a configuration file, and the
runnumbers you want to use and submits them in batches to Condor. (It's best
to run it in a `tmux` or `screen` session.) It will wait while your jobs run
and periodically check on their progress, submitting more jobs as they
complete. This helps you be a good citizen in your use of Condor by avoiding
having too many jobs running at once. It will even move your jobs from their
running device (usually `/data2`) to a storage device inaccessible to Condor
(e.g., `/data6`)!

### Exploring the Jobs
Leave [`submit_to_condor.py`](submit_to_condor.py) running for now. In another
shell, do:
...
> > `/data2/<your username>/work/mytestjob/`, which just helps organize the
> > many `WORK_DIR` and will not prevent the job from running.

Once the job completes, [`run_stages.py`](run_stages.py) (the script that
[`submit_to_condor.py`](submit_to_condor.py) submits to Condor) moves the
output from the `WORK_DIR` to the `DATA_DIR` and `LOG_DIR`. From there,
[`submit_to_condor.py`](submit_to_condor.py) picks the output up and moves it
...
[`run_stages.py`](run_stages.py) what to do. The easiest way to write them is
to use a template (see __Using Templates__ below). Once you are confident your
configuration file works the way you want it to (see __Testing__ below), you
pass it as an argument to either [`run_stages.py`](run_stages.py) or
[`submit_to_condor.py`](submit_to_condor.py) (which passes it to
[`run_stages.py`](run_stages.py)).

> ___DO NOT CHANGE YOUR CONFIGURATION FILE while___
> [`submit_to_condor.py`](submit_to_condor.py) ___runs.___ This will result in
> your jobs' behavior changing mid-submission!

You are encouraged to add your own command-line arguments to your
configuration file using the
[`argparse`](https://docs.python.org/2.7/library/argparse.html) module in
python. You can see an example in
[`samples/configuration_files/complex.py`](complex.py)
...
You can use command-line arguments from your configuration file with both
[`run_stages.py`](run_stages.py) and
[`submit_to_condor.py`](submit_to_condor.py). They know how to handle them.

You can read more about [`run_stages.py`](run_stages.py) and
[`submit_to_condor.py`](submit_to_condor.py) in the
...
edits, and you're ready to go.

> Note you will need to overwrite the `<<<<` and `>>>>`&mdash;do not leave them
> in the file! You'll know you've made all the necessary changes when `<<<<`
> and `>>>>` no longer appear in the file.

See [templates/TEMPLATES.md](TEMPLATES.md) for more information.
...
submit_to_condor.py --test <jobname> <path/to/configfile>
--setlohi 100000 100002`, wait for it to finish, then check the output. If
everything is as-expected, you can go ahead and submit for real using
[`submit_to_condor.py`](submit_to_condor.py).

> Remember that [`run_stages.py`](run_stages.py) does not know what you are
> trying to do. It's up to you to make sure your scripts actually work and
> that any environments you use (e.g., a local Gauss build) are built properly.
...
```
`sys` is the absolute path to where files will be stored or run (usually
`/data2` or `/data6`). `jobnum` is the run-number for the `jobname`.

Scripts run in the work directory, data files are stored in the data directory,
and log files are stored in the log directory. The scripts in this suite rely
on this structure and ___will not work___ without it. Keep this in mind,
particularly when developing.

### run_stages.py
At the heart of SU-lhcb-MC-gen is [`run_stages.py`](run_stages.py). This does
most of the leg-work to run scripts. [`run_stages.py`](run_stages.py) centers
around a function in the configuration file called `make_stage_list`, which
describes the scripts to be run, how, and in what order. If you do not want to
submit in-batch, you can run [`run_stages.py`](run_stages.py) on its own.

[`run_stages.py`](run_stages.py) is script-agnostic. You can run whatever sort
of code you like using it, so long as the code fits in the `make_stage_list`
framework.
...
```python
def make_stage_list(USER, BASE_NAME):  # DO NOT CHANGE THIS LINE; run_stages.py will pass its USER and BASE_NAME ('<SIGNAL_NAME>_<RUN_NUMBER') parameters--you may refer to them inside this function if you want to
    stage_list = []
    stage_list.append(
        {
            ...
            'required': ['filenames_needed_to_run_this_stage'],  # list stuff required for the stage to run here, generally data files produced by earlier stages. run_stages.py will fetch them from the DATA_DIR if they are not in the WORK_DIR already and throw an error if they are not found (unless the SOME_MISSING option is used, as when running with submit_to_condor.py [which always uses this option], in which case no error is thrown--the stage is skipped and no error is thrown)
...
This function determines everything that happens in your job.
[`run_stages.py`](run_stages.py) will run the `'call_string'` (along with some
environment-setting parameters) inside a fresh `tcsh` shell for each stage
sequentially. This means that each stage can refer to and use the output of
previous stages.

> [`run_stages.py`](run_stages.py) is script agnostic, meaning it does not
> care what you tell it to run. This means it's your responsibility to make
> sure each stage's 'call_string' somehow refers to 'scripts' and picks up the
> previous stages' output, if you want it to.
...
#### arguments
A number of arguments are essential to [`run_stages.py`](run_stages.py) and
they must appear in your configfile. There are, in theory, other ways to
declare these parameters besides using argparse, but this method is highly
recommended. See the help string for each argument to understand what it does.
```python
...
parser.add_argument('configfile', type=os.path.abspath,
                    help='this argument must be here to ensure integration with run_stages.py; does not do anything')
parser.add_argument('--SIGNAL_NAME', default='TestProduction',
                    help='what you want the organizing directory for your job to be named. submit_to_condor.py sets this parameter for you when it submits jobs.')
...
cleangroup.add_argument('--noCLEANSTAGES', dest='CLEANSTAGES', action='store_false',
                        help='CLEANSTAGES deletes "to_remove" files from earlier stages as it goes.')
cleangroup.add_argument('--noCLEANWORK', dest='CLEANWORK', action='store_false',
                        help='CLEANWORK moves files out of WORK_DIR.')
cleangroup.add_argument('--SOME_MISSING', action='store_true',
                        help='Lets the script terminate without errors if the "required" files are missing. Useful for running later stages. Used automatically by submit_to_condor.py')
...
debuggroup.add_argument('--WORK_DIR_EXISTS', action='store_true',
                        help='BE VERY CAREFUL WHEN USING THIS FLAG: gives permission to run if WORK_DIR already exists! Your files may be overwritten.')
...
[`run_stages.py`](run_stages.py) won't care about them, but your configfile
will still see them. [`submit_to_condor.py`](submit_to_condor.py) is even
written with this in mind&mdash;
`submit_to_condor.py <jobname> <path/to/configfile> <your arguments>`
should work just fine, as long as your arguments don't conflict with the ones
used by this suite.
...
### [submit_to_condor.py](submit_to_condor.py)
This script is built around [`moveFiles.py`](moveFiles.py). It uses the same
commandline arguments (plus some others) but reappropriates them so that files
can be moved around [`run_stages.py`](run_stages.py), which
[`submit_to_condor.py`](submit_to_condor.py) runs on Condor in batches.
...
__This is the heart of SU-lhcb-MC-gen__. Once you've tested your configfile,
you should only have to run this script (if nothing goes awry).

#### `--chunks_of` and `--lessthan`
These are the main parameter that allows you to be a good citizen on Condor.
Together, they specify the maximum number of jobs you want to have on Condor
and `run_sys` at once. The default values should be fine most of the time, but
if you have particularly slow jobs (that take more than two hours to run, say),
you should consider reducing them. If your jobs are particularly fast (say,
less than 10 minutes), you can use larger values.

> `--chunks_of` specifies how many jobs
> [submit_to_condor.py](submit_to_condor.py) should submit at a time.
>
> `--lessthan` specifies how few jobs the user should be running on Condor
> before submitting again.
>
> So if you have only one job running, you will never
> have more than `lessthan` + `chunks_of` running at once. If you have multiple
> jobs running (with the same `lessthan` and `chunks_of` values), you will
> never have more than `lessthan` + Njobs*`chunks_of` jobs running at once.

#### `--setlohi` and `--runfromstorage`
When you use [`submit_to_condor.py`](submit_to_condor.py), you must use one of
these two options.
...
> __NB:__ If you are using [`submit_to_condor.py`](submit_to_condor.py) to
> submit Monte Carlo, this is _not_ equal to the number of events. You will
> end up with a number of events equal to (hi - lo) * `NUM_EVENT`, where
> `NUM_EVENT` is the number of events generated in each job
...
this data to the `run_sys`, where [`run_stages.py`](run_stages.py) will be
able to get to it.

> [`run_stages.py`](run_stages.py) looks at the `'required'` key in each
> stage's dictionary and moves the files listed there from the `DATA_DIR` to
> the `WORK_DIR` so that they are available for the stage to use. Don't worry
> &mdash; they get put back into the `DATA_DIR` just as though you had listed
> them as data files.
...
[`samples/configuration_files/basic_MCGen_2016_s28r1_WithArguments.py`](basic_MCGen_2016_s28r1_WithArguments.py)
. You could generate just through the `Brunel` stage by doing:
...
#### What if [`submit_to_condor.py`](submit_to_condor.py) quits?
...
> run. [`submit_to_condor.py`](submit_to_condor.py) just handles the submission process.
